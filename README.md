...
`게시판에서 한강 검색해줘`처럼 질문하면 `intent`가 `post_search`로 반환되고,
`post_results`에 게시글 ID·제목·본문 미리보기·조회수·작성일이 포함됩니다.
모든 화면 우측 하단에서 서울 도우미를 열 수 있습니다. 대화 기록, 추천 질문,
지역 장소·게시글 결과 카드를 제공하며 680px 이하에서는 전체 화면으로 표시됩니다.
